...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(26):
            case = lowercases[i]
            shift_case_index = (i + shift) % 26
            d[case] = lowercases[shift_case_index]
        for i in range(26):
            case = uppercases[i]
            shift_case_index = (i + shift) % 26
            d[case] = uppercases[shift_case_index]
        return d
...
class PlaintextMessage(Message):
    ...
    def get_encrypting_dict(self):
        """
        Used to safely access a copy self.encrypting_dict outside the class
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        """
        maximum_number, that_shift = 0, 0
        word_list = self.get_valid_words()
        for i in range(26):
            n = 0
            shift_msg_list = self.apply_shift(i).split()
            for j in shift_msg_list:
                if is_word(word_list, j):
                    n += 1
            if n > maximum_number:
                maximum_number = n
                that_shift = i
        return that_shift, self.apply_shift(that_shift)


def decrypt_story():
    s = get_story_string()
    msg = CiphertextMessage(s)
